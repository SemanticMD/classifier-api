...
class Classifier(caffe.Net):
    ...
    def classify_one(self, image):
        ...
        resized_image = caffe.io.resize_image(image, self.image_dims)

        preprocessed = self.transformer.preprocess(
            'data', resized_image)

        # reshape net input (if necessary)
        test_shape = (1,) + preprocessed.shape
        if self.blobs['data'].data.shape != test_shape:
            self.blobs['data'].reshape(*test_shape)

        # run inference
        self.blobs['data'].data[...] = preprocessed
        output = self.forward()
        scores = output[self.outputs[-1]].flatten()
